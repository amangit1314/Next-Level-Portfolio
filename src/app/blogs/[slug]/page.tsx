...
const BlogPostPage = () => {
    ...
    const portableTextComponents = {
                ...
                <div className="relative w-full h-96 my-8 rounded-xl overflow-hidden border border-zinc-800">
                    ...
                    {value.caption && (
                        <div className="absolute bottom-0 left-0 right-0 bg-black/70 backdrop-blur-sm p-3">
                            <p className={`text-sm text-zinc-300 text-center ${inter.className}`}>
                                {value.caption}
                            </p>
                        </div>
                    )}
        ...
        block: {
            ...
            h4: ({ children }: any) => (
                <h4 className={`text-xl font-bold text-white mb-3 mt-6 ${unbounded.className}`}>
                    {children}
                </h4>
            ),
            normal: ({ children }: any) => (
                <p className={`text-zinc-300 text-lg leading-relaxed mb-6 ${inter.className}`}>
                    {children}
                </p>
            ),
            blockquote: ({ children }: any) => (
                <blockquote className="border-l-4 border-purple-500 pl-6 my-6 italic">
                    <p className={`text-zinc-400 text-lg ${inter.className}`}>{children}</p>
                </blockquote>
            ),
        },
        marks: {
        ...
        list: {
            bullet: ({ children }: any) => (
                <ul className={`list-disc list-inside space-y-2 mb-6 text-zinc-300 ${inter.className}`}>
                    {children}
                </ul>
            ),
            number: ({ children }: any) => (
                <ol className={`list-decimal list-inside space-y-2 mb-6 text-zinc-300 ${inter.className}`}>
                    {children}
                </ol>
            ),
        },
    ...
    const authorImageUrl = blog.authorImage?.asset?.url || profile?.profileImage?.asset?.url;
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-4 py-12">
                    ...
                    >
                        <div className="flex flex-wrap items-center gap-3 mb-6">
                            ...
                            {blog.featured && (
                                <span className={`px-3 py-1 rounded-full text-xs font-bold text-white bg-gradient-to-r from-purple-500 to-pink-500 ${unbounded.className}`}>
                                    Featured
                                </span>
                            )}
                        </div>

                        <h1 className={`text-4xl md:text-5xl font-black text-white mb-6 leading-tight ${unbounded.className}`}>
                            {blog.title}
                        </h1>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 text-sm">
                                        ...
                                        <span className={`text-zinc-300 ${unbounded.className}`}>{authorName}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <motion.div
                        ...
                        className="mt-16 p-8 bg-gradient-to-br from-purple-900/20 to-pink-900/20 rounded-2xl border border-purple-500/20"
                    >
                        <h3 className={`text-2xl font-bold text-white mb-4 ${unbounded.className}`}>
