...
- **2026-08-25 — rotateX tilt reinstated on list rows, deliberately reversing an earlier removal.** `BlogListRow.tsx` once carried a comment saying a pre-HUD card-flip/3D-tilt treatment was removed as off-brand. After explicit user approval (reference: saifullah.dev/projects), `useScrollReveal`'s new `"tilt-rows"` variant reintroduces a *different* effect — continuous per-row scrub tilt (not a hover flip) — on `ProjectListRow`/`BlogListRow`/`ComponentListRow`. If a future session finds a comment claiming "no 3D tilt, off-brand for this HUD identity," check the git history/date before trusting it — this file's own comment was already wrong once. `tilt-rows` builds its own per-item GSAP timeline (documented inline in the hook) rather than reusing the shared fromTo path the other 3 variants use — a real, intentional divergence, not an inconsistency to "fix." New shared primitive `src/components/primitives/HudMediaFrame.tsx` (scanline + corner brackets, Framer since it's non-scroll/continuous) now backs all 3 rows' thumbnails — reuse it for any future indexed-list row, don't hand-roll another bordered `<Image>`.
